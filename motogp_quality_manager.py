"""Finales Domain-QM für Racing-Pakete vor Chief-QM und Telegram."""
import re
...
def review(item,caption):
 ...
 # Sprachunabhängige Bindung: erkannter Fahrer oder konkrete Zahl/Ort muss Quelle und Caption verbinden.
 source_riders=[name for name in RIDERS.values() if fold(name) in source]
 caption_riders=[name for name in RIDERS.values() if fold(name) in low]
