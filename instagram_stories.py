...
def find_story_block(content):
    ...
    for match in re.finditer(pattern, content, re.DOTALL):
        ...
        if image_match:
            return image_match.group(1).strip(), block
    return None, None

def create_story(ig_user_id, token, image_url):
...
if __name__ == "__main__":
    ...
    image_file, block = find_story_block(content)
    if not image_file:
        print("Keine freigegebene Story gefunden.")
        exit(0)

    image_url = REPO_RAW + image_file
    print(f"Story gefunden – veröffentliche {image_file}...")

    creation_id = create_story(ig_user_id, token, image_url)
    if not creation_id:
        exit(1)
