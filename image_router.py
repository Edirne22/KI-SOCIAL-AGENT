        ...
        for model in NVIDIA_MODELS:
            ...
            for attempt in range(4):  # initial attempt + up to 3 retries
                try:
                    ...
                    logger.error(f"image_router: Network error for model {model}: {e}")
                    model_failed = True
                    break
                ...
                if response.status_code == 200:
                    try:
                        ...
                        logger.info(f"image_router: generated via {model}")
                        return img_bytes
                    except (KeyError, IndexError, TypeError, ValueError) as err:
                        logger.error(f"image_router: Failed to parse response from {model}: {err}")
                        model_failed = True
                        break
                    ...
                    if attempt < 3:
                        ...
                        logger.warning(
                            f"image_router: Rate limit 429 for model {model}. Retrying in {delay}s (attempt {attempt + 1}/3)..."
                        )
                        time.sleep(delay)
                    else:
                        logger.warning(
                            f"image_router: Rate limit 429 persisted for model {model} after 3 retries."
                        )
                        model_failed = True
                        break
                    ...
                    logger.error(
                        f"image_router: Server error {response.status_code} for model {model}."
                    )
                    model_failed = True
                    break
                    ...
                    logger.error(
                        f"image_router: HTTP error {response.status_code} for model {model}: {response.text[:200]}"
                    )
                    model_failed = True
                    break
