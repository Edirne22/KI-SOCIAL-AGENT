...
def handle_one(update_id: int, chat_id: str, message_text: str, message: dict | None = None) -> bool:
...
def main() -> None:
    if len(sys.argv) >= 4:
        ...
        chat = sys.argv[2]
        text = sys.argv[3]
        handle_one(uid, chat, text, message={"date": 0})
        acknowledge_through(uid)
        return
    ...
    for update in sorted(updates, key=lambda item: item.get("update_id", 0)):
        ...
        if handle_one(update_id, str(chat.get("id", "")), message_text, message):
            acknowledge_through(update_id)
            return
