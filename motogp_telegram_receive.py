...
import re
from telegram_bot import get_chat_id, get_updates, get_updates as acknowledge, send_message
...
def publish(posts,chosen,update_id):
 ...
 for n in chosen:
  p=posts.get(n)
  if not p:continue
  # Instagram: eigenes/auto-freigegebenes Medium. Facebook: offizielle URL im Text erzeugt Link-Preview, wenn Meta dies zulässt.
...
def main():
 ...
 for upd in sorted(get_updates(),key=lambda x:x.get('update_id',0)):
  ...
  if not isinstance(uid,int) or chat!=allowed or not isinstance(txt,str):continue
  chosen=selection(txt)
  if chosen is None or already(uid):continue
  if chosen:
   publish(posts,chosen,uid);send_message(f'✅ MotoGP: {len(chosen)} Content-Paket(e) freigegeben. Instagram und Facebook übernehmen jetzt über die bestehende Publisher-Kette.')
  else:send_message('❌ MotoGP-Tagesauswahl verworfen. Es wird nichts veröffentlicht.')
  STATE.parent.mkdir(parents=True,exist_ok=True);STATE.write_text(f'Update-ID: {uid}\nAntwort: {txt}\n',encoding='utf-8');acknowledge(offset=uid+1)
