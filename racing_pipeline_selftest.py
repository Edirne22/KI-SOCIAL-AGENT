...
def test_freshness_and_semantic_fail_closed_contracts():
    ...
    assert_true(a.current_news({'url':'https://example.com/2026/09/15/story'},now,7),'fresh dated URL rejected')
    assert_true(not a.current_news({'url':'https://example.com/2026/08/01/story'},now,7),'stale story accepted')
    assert_true(not a.current_news({'url':'https://example.com/story'},now,7),'undated story must fail closed')
